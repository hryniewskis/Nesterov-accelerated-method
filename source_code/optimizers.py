...
class Nesterov_Optimizers:
    ...
    def __m_L(self, y, x, L):
        return (
            self.__function_f(y)
            + np.dot(self.__gradient_f(y), x - y)
            + (L / 2) * norm(x - y) ** 2
            + self.__function_Psi(x)
        )
    ...
    def __stopping_optimal_cond(self, x, verbose: bool):
        ...
        if np.any(np.abs(x) <= np.finfo(float).eps):
            subgrad_Psi = np.where(
                np.abs(x) <= np.finfo(float).eps,
    ...
    def __dual_gradient_method(self, x, L, verbose: bool):
        ...
        for it in range(self.max_iter):
            ...
            psi_b = psi_b + (1.0 / M) * self.__gradient_f(v)
            psi_d = psi_d + (1.0 / M) * lambda_
    ...
    ### Another implementation of accelerated method
    ...
    def fit(
        ...
        assert X.shape[0] == len(
            y
        ), "number of rows in X has to be the same as length of y"
